    ...
    for index, row in data.iterrows():
        ...
        if existed_df is not None and not single_article and not force_reprocess:
            ...
            if not existed_df.at[article_url, 'ner_process'] == ner_process:
                diff_ner = (article_url, existed_df.at[article_url, 'ner_process'])
                break
        else:
...
if correct_file_type:
    ...
    if not language_selected == '<SELECT>' and selection_made:
        ...
        if selected_display == 'Aggregate':
            ...
            if diff_processing is not None:
                message = "Hello there! Good news, {} within the datafile selected has already been run using {}. " \
                          "Would you like to reprocess using the inputted NER Process?".format(diff_processing[0], diff_processing[1])
                st.write(message)
